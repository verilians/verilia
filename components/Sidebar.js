  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 left-4 z-50 p-2 rounded-lg bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700 md:hidden"
      >
              ...
              {menuItems.map((item) => (
                <div key={item.id} className="w-full">
                  <button
                    ...
                    className="w-full flex items-center space-x-3 p-3 rounded-lg text-left hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                  >
                        ...
                        {isLoading ? (
                          ...
                          chats.map((chat) => (
                            <div
                              ...
                              className={`px-3 py-2 cursor-pointer transition-colors hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg ${
                                currentChatId === chat.id ? 'bg-purple-50 dark:bg-purple-900/20 border-l-2 border-purple-500' : ''
                              }`}
                            >
                              <div className="flex items-center justify-between">
                                ...
                                <button
                                  ...
                                  className="ml-2 p-1 rounded-md hover:bg-red-100 dark:hover:bg-red-900/20 transition-colors opacity-0 group-hover:opacity-100"
                                  title="Delete chat"
                                >
      ...
      {/* Desktop Sidebar */}
      <div className={`${isExpanded ? 'w-64' : 'w-16'} bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col h-full transition-all duration-300 ease-in-out hidden md:flex`}>
        {/* Toggle Button */}
        ...
        <div className="flex flex-col items-center lg:items-start py-4 space-y-4 flex-1">
          {menuItems.map((item) => (
            <div key={item.id} className="w-full">
              ...
              {/* Desktop Chat List for Chats tab */}
              {item.id === "chats" && isChatsExpanded && user && isExpanded && (
                <div className="mt-2 ml-4 border-l border-gray-200 dark:border-gray-600 pl-4">
                  {/* Chat List */}
                  <div className="space-y-1">
                    {isLoading ? (
                      ...
                      chats.map((chat) => (
                        <div
                          key={chat.id}
                          onClick={() => handleChatSelect(chat.id)}
                          className={`px-3 py-2 cursor-pointer transition-colors hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg group ${
                            currentChatId === chat.id ? 'bg-purple-50 dark:bg-purple-900/20 border-l-2 border-purple-500' : ''
                          }`}
                        >
                          <div className="flex items-center justify-between">
